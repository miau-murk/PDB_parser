...
class MorganTypes:
    def __init__(self, m):
        ...
        blok = Chem.MolToMolBlock(m) 
        

        self._name, self.xyz, self.graph = read_blok(blok, m) # dict, dict
    ...
    def calc_morgan_weight(self, k=10, str=7, sts=9, flag_ze=True, flag_indster=False):
        ...
        for i in range(k):
            ...
            for i in self.mgc.keys():
                if self.xyz[i][1] != "H":
                    molecule = Chem.MolFromSmiles(self.xyz[i][1])
                    for atom in molecule.GetAtoms():
                        atomic_weight = Chem.GetPeriodicTable().GetAtomicWeight(atomic_number)
                    self.mgc[i] += atomic_weight
